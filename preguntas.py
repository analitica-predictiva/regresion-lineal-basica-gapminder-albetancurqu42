...
def pregunta_02():
    ...
    print(f'{df.loc[:, ["life", "fertility"]].corr(method="pearson").iloc[1, 0]:.4f}')

    # Imprima la media de la columna `life` con 4 decimales.
    print(f'{df.loc[:, ["life"]].mean().iloc[0]:.4f}')

    # Imprima el tipo de dato de la columna `fertility`.
    print(type(df.loc[:, "fertility"]))

    # Imprima la correlación entre las columnas `GDP` y `life` con 4 decimales.
    print(f'{df.loc[:, ["GDP", "life"]].corr(method="pearson").iloc[1, 0]:.4f}')
